...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if (!$room_meta) {
        header("Location: accommodations.php?error=not_found");
        exit;
    }
    ...
    if ($action === 'set_maintenance') {
        ...
        header("Location: accommodations.php?success=1");
        exit;
    }

    if ($action === 'clear_maintenance') {
        if (sf_room_has_checked_in_booking($conn, $room_id)) {
            header("Location: accommodations.php?error=occupied");
            exit;
        }
...
<body>
    <?php $active_page = 'accommodations'; include '_sidebar.php'; ?>
                        ...
                        <td>
                            <div class="status-action">
                                <?php if ($status === 'maintenance'): ?>
                                    ...
                                    <?php else: ?>
                                        <form method="POST" action="accommodations.php" style="margin:0;">
