...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'checkin') {
    ...
    header("Location: admin_checkin?success=1" . $print_params);
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'walkin_checkin') {
    ...
    if (!empty($guest_name) && $room_id > 0) {
        ...
        if ($room_info) {
            ...
            header("Location: admin_checkin?success=walkin" . $print_params);
            exit;
        }
    }
}
...
<head>
    ...
    <style>
        ...
        .sc-close-btn:hover { color: #374151; }
        .sc-icon {
            width: 60px;
            height: 60px;
            background: #DBEAFE;
        ...
        .sc-details strong { color: #111827; }
...
<body>
    ...
    <main class="admin-main">
        ...
        <section class="dashboard-grid" style="grid-template-columns: 1fr;">
            ...
            <?php if ($specific_booking): ?>
                ...
                ?>
                <div class="specific-checkin-overlay">
                    <div class="specific-checkin-card">
                        <a href="admin_checkin" class="sc-close-btn" title="Cancel scan">✕</a>
                        
                        <div class="sc-icon">📱</div>
                        <h2 style="margin: 0 0 5px; font-size: 22px; color: #111827;">Guest Scanned!</h2>
                        <p style="margin: 0; color: #6B7280; font-size: 14px;">Booking found successfully.</p>
                        
                        <div class="sc-details">
                            <p><strong>Guest Name:</strong> <?php echo $spec_name; ?></p>
                            <p><strong>Accommodation:</strong> <?php echo $spec_acc; ?></p>
                            <p><strong>Booking Ref:</strong> <?php echo htmlspecialchars($ref); ?></p>
                        </div>
                        
                        <?php if ($spec_balance > 0): ?>
                            <div style="background: #FEF2F2; border: 1px solid #FCA5A5; border-radius: 12px; padding: 16px; margin: 0 0 20px; color: #991B1B; text-align: center;">
                                <h3 style="margin: 0 0 4px; font-size: 18px;">⚠️ Balance Due: ₱<?php echo number_format($spec_balance, 2); ?></h3>
                                <p style="font-size: 13px; margin: 0; color: #B91C1C;">Please collect the balance payment before check-in.</p>
                            </div>
                        <?php else: ?>
                            <div style="background: #ECFCCB; border: 1px solid #BEF264; border-radius: 12px; padding: 12px; margin: 0 0 20px; color: #3F6212; text-align: center;">
                                <strong>✅ Fully Paid</strong>
                            </div>
                        <?php endif; ?>
                        
                        <form method="POST" action="admin_checkin" style="margin: 0;">
                            <input type="hidden" name="action" value="checkin">
                            <input type="hidden" name="booking_id" value="<?php echo $spec_id; ?>">
                            <button type="submit" class="btn-checkin" style="width: 100%; font-size: 16px; padding: 14px; border-radius: 10px;" onclick="handleCheckinClick(event, <?php echo $spec_id; ?>, '<?php echo addslashes($spec_name); ?>', '<?php echo addslashes($spec_acc); ?>', <?php echo $spec_total; ?>, <?php echo $spec_balance; ?>)">Check In Guest Now</button>
                        </form>
                    </div>
                </div>
                    ...
                    <tbody>
                        ...
                        if ($bookings_query && $bookings_query->num_rows > 0) {
                            while ($row = $bookings_query->fetch_assoc()) {
                                ...
                                echo "<td>
                                    <form method='POST' action='admin_checkin' style='display:inline;'>
                                        <input type='hidden' name='action' value='checkin'>
    ...
    <div class="gcash-overlay" id="walkinModalOverlay">
        <div class="gcash-modal" style="max-width: 580px;">
            ...
            <form method="POST" id="walkinForm" action="admin_checkin">
                ...
                <div style="display:grid; grid-template-columns: 1fr 1fr; gap: 12px; margin-bottom: 14px;">
                    <div style="grid-column: span 2;">
                        ...
                        <input type="text" name="guest_name" id="walkinGuestName" required placeholder="e.g. Maria Santos" style="width:100%; padding:10px 12px; border:1.5px solid #D1D5DB; border-radius:8px; font-size:14px; box-sizing:border-box;">
                    </div>
                    <div>
    ...
    <div class="gcash-overlay" id="balanceModalOverlay">
        <div class="gcash-modal">
            ...
            <form method="POST" id="balanceForm" action="admin_checkin">
                <input type="hidden" name="action" value="checkin">
...
function handleCheckinClick(event, bookingId, guestName, accName, totalCost, balanceDue) {
    if (balanceDue > 0.01) {
        ...
        // Hide the scanned guest overlay if it's open
        const scannedOverlay = document.querySelector('.specific-checkin-overlay');
        if (scannedOverlay) scannedOverlay.style.display = 'none';
...
function closeBalanceModal() {
    ...
    // Bring back the scanned guest overlay if they cancel the payment
    const scannedOverlay = document.querySelector('.specific-checkin-overlay');
    if (scannedOverlay) scannedOverlay.style.display = 'flex';
}
