...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($action === 'mark_read' && isset($_POST['notif_id'])) {
        ...
        $stmt = $conn->prepare("
            SELECT b.guest_name, b.guest_email, b.accommodation_name, b.check_in, b.check_out, b.cancellation_token,
        ...
        if ($result && $result->num_rows > 0) {
            ...
            $cancellation_url = rtrim($base_url, '/') . '/cancel_booking.php?token=' . $cancellation_token;
...
<body>
    ...
    <main class="admin-main">
        ...
        $header_extra_html = '
            <form method="POST" action="admin_notifications.php" style="margin: 0;">
        ...
        <section class="dashboard-grid" style="grid-template-columns: 2fr 1fr;">
                        ...
                        <?php while ($n = $notifs_query->fetch_assoc()): ?>
                            ...
                            <div class="notif-item type-<?php echo $type; ?> <?php echo ($is_read ? '' : 'unread'); ?>">
                                ...
                                <div class="notif-actions">
                                    <?php if (!empty($n['booking_id']) && !$is_read): ?>
                                        <form method="POST" action="admin_notifications.php" style="margin:0;" onsubmit="return false;" data-confirm-form>
                                            <input type="hidden" name="action" value="accept_booking">
                                            <input type="hidden" name="notif_id" value="<?php echo $id; ?>">
                                            <input type="hidden" name="booking_id" value="<?php echo (int)$n['booking_id']; ?>">
                                            <button type="button" class="btn-action-icon" style="background:#4CAF50; color:white; border-color:#4CAF50;" title="Accept booking and email guest" onclick="openAcceptModal(this.closest('form'))">Accept</button>
                                        </form>
                                    <?php endif; ?>
                                    <?php if (!$is_read): ?>
                                        <form method="POST" action="admin_notifications.php" style="margin:0;">
            ...
            <div class="card">
                ...
                <form method="POST" action="admin_notifications.php" style="display: flex; flex-direction: column; gap: 15px; margin-top: 15px;">
